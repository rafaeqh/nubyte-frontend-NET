...
namespace WebApplicationBlowout.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginViewModel model, string returnUrl)
        {
            ...
            String datosLogin = serviciosWeb.login(model.Email, model.Password);
            Char delimiter = ' ';
            String[] substrings = datosLogin.Split(delimiter);

            if (datosLogin.Equals("-1"))
            {
                ModelState.AddModelError("", "Usario o Contraseña no válido.");
                return View(model);
            }
            else {
                ...
                {
                    Session["login"] = "1";
                    Session["Nombre"] = substrings[0];
                    Session["Correo"] = model.Email;
                    Session["NIT"] = substrings[3];
                    ...
                    //return RedirectToAction("ViewAdministrador", "Shared");
                }
                else
                {
                    if (substrings[2].Equals("2"))
                    {
                        Session["login"] = "2";
                        Session["Nombre"] = substrings[0];
                        Session["Correo"] = model.Email;
        ...
        public ActionResult Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                Boolean result = serviciosWeb.crearCuenta(model.cedula, model.nombre, model.Email, model.Password, "1", Session["NIT"].ToString());
                if (result)
                {
